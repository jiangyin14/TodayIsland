...
public partial class LunarDateControl : ComponentBase
{
    ...
    private async void LoadLunarDateAsync()
    {
        await Task.Run(() =>
        {
            ...
            var monthText = GetMonthName(realMonth);
            if (isLeap)
            {
            ...
            var lunarDate = $"农历 {chineseEra}年{monthText}{dayText}";

            Dispatcher.UIThread.InvokeAsync(() => LunarDate.Text = lunarDate);
        });
    }
    ...
    private string GetMonthName(int month)
    {
        switch (month)
        {
            ...
            case 11: return "冬月";
            case 12: return "腊月";
            default: return ConvertToChinese(month) + "月";
        }
    }

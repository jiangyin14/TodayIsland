...
public partial class NextHolidayDateEasierControl : ComponentBase
{
    ...
    private async void LoadNextHolidayEasierAsync()
    {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var streamReader = new StreamReader(responseStream))
                {
                    ...
                    if (!IsValidJson(responseBody))
                    {
                        ...
                        Dispatcher.UIThread.InvokeAsync(() => NextHolidayDateEasier.Text = "加载节假日信息失败");
                        return;
                    }
                    ...
                    if (json["next_holiday"] != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(name) && !string.IsNullOrEmpty(countdown))
                        {
                            ...
                            Dispatcher.UIThread.InvokeAsync(() => NextHolidayDateEasier.Text = formattedText);
                        }
                        else
                        {
                            Dispatcher.UIThread.InvokeAsync(() => NextHolidayDateEasier.Text = "未找到节假日信息");
                        }
                    }
                    else
                    {
        ...
        catch (Exception e)
        {
            ...
            Dispatcher.UIThread.InvokeAsync(() => NextHolidayDateEasier.Text = "加载节假日信息时发生未知错误");
        }
    }
